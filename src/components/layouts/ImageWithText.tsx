...
export default function ImageWithText({ children, image, images, textRight, textBoxClass, hideImageOnMobile }: any) {
  const isMobile = window.matchMedia("(max-width: 768px)").matches;
  const hideImage = hideImageOnMobile && isMobile;
  return (
    <div className='w-[100vw] h-[100vh] flex mb:inline mb:h-auto overflow-hidden'>
      ...
      {image && !hideImage && <div className={'w-[50vw] bg-cover bg-no-repeat bg-center h-full mb:w-full mb:h-[100vh]'} style={{ backgroundImage: `url(${image})` }}></div>}
    </div>
  );
}
